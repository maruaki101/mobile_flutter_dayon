...
Sake Stopは、飲食店のテーブルごとに注文を記録し、利用者自身が純アルコール摂取量と注文ペースを確認するFlutterアプリです。注文履歴はFirebase Realtime Databaseへ保存・同期します。
...
- 会計終了時のセッションクローズと画面リセット

## 利用手順

1. テーブルのQRコードを読み取るか、テーブルIDを入力します。
2. 必要に応じてニックネームを入力し、「開始」を選びます。
3. 商品をカートへ追加し、カート画面で数量を確認して注文を確定します。
4. 摂取量と注文履歴を確認し、退店時に「会計・終了」を選びます。
...
注文確定時は、カート内の各商品を1個につき1履歴レコードとして、`orders`配下へ一括更新します。
